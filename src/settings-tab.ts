...
export class LinkPreviewSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    containerEl.empty();
    containerEl.createEl('h2', { text: 'Link preview card' });
  ...
  private async saveNumber(key: 'cacheMinutes' | 'maxCacheEntries' | 'maxDescriptionLength' | 'maxContentLength' | 'maxGalleryImages', value: string, min: number, max: number): Promise<void> {
  ...
  private toggle(container: HTMLElement, name: string, desc: string, key: keyof LinkPreviewPlugin['settings']): void {
    new Setting(container).setName(name).setDesc(desc).addToggle((toggle) => toggle.setValue(this.plugin.settings[key] as boolean).onChange(async (value) => { this.plugin.settings[key] = value as never; await this.plugin.saveSettings(); }));
  }
}
